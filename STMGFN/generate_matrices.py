...
def generate_dtw_spa_matrix(dataset_name, in_len, out_len, sigma1=0.1, thres1=0.6, sigma2=10, thres2=0.5, re_scale=True,topk=3):
    ...
    topk=topk
    adj = dist_matrix+ dist_matrix.T
    dtw_adj = np.zeros((num_node, num_node))
    for i in range(adj.shape[0]):
        a = adj[i,:].argsort()[0:topk]
        for j in range(topk):
            dtw_adj[i, a[j]] = 1
  
    for i in range(num_node):
        for j in range(num_node):
            if (dtw_adj[i][j] != dtw_adj[j][i] and dtw_adj[i][j] ==0):
                dtw_adj[i][j] = 1
            if( i==j):
                dtw_adj[i][j] = 1
    ...
    if dataset_name in ["PEMS03", "PEMS04", "PEMS07", "PEMS08"]:
        ...
        sp_matrix = np.load('{0}/{1}_spatial_distance.npy'.format(os.path.abspath(__file__ + "/.."), dataset_name))
        # normalization
    ...
    print(f'average degree of semantic graph is {np.sum(dtw_adj > 0)/2/num_node}')
    # normalize
 
    dtw_matrix = [calculate_transition_matrix(dtw_adj)]#,calculate_transition_matrix(dtw_adj).T]
    sp_matrix = [calculate_transition_matrix(sp_matrix)]#,calculate_transition_matrix(sp_matrix).T]
